 ...
 * @brief Apply Fusion AHRS algorithm to BMI160 sensor data
 * 
 * This program reads BMI160 IMU data from bmi160.csv, applies the Fusion
 * sensor fusion algorithm, and outputs the fused orientation to fusioned.csv
 */
...
constexpr float GYRO_SENSITIVITY = 16.4f;
...
int main() {
    ...
    // Write CSV header
    outputFile << "time,roll,pitch,yaw,qw,qx,qy,qz,linear_accel_x,linear_accel_y,linear_accel_z" << std::endl;
    ...
    FusionAhrsSettings settings = {
        ...
        .recoveryTriggerPeriod = 5 * (int)(1.0f / SAMPLE_PERIOD)  // 5 seconds worth of samples
    };
    ...
    while (std::getline(inputFile, line)) {
        ...
        float deltaTime = (sampleCount == 0) ? SAMPLE_PERIOD : (currentTime - prevTime);
        if (deltaTime <= 0.0f) {
    ...
    std::cout << "Fusion completed! Processed " << sampleCount << " samples." << std::endl;
