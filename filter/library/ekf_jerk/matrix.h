 ...
 *          1. A(idxRow,idxCol)       <-- The preferred way. With bounds
 * checking.
 *          2. A[idxRow][idxCol]      <-- Slow, not recommended, but makes a
 * cute code. With bounds checking.
 *          3. A._at(idxRow,idxCol)   <-- Just for internal function usage.
 ...
 *      - Add NoInitMatZero for _outp matrix initialization in
 * ForwardSubtitution.
 *      - Fixing bug in HouseholderTransformQR function where _outp & _vectTemp
 * need to be initialized with zero.
 *      - Change comparison with zero in these functions using
 * float_prec_ZERO_ECO:
 *          -> operator ==
 *          -> operator /
 ...
 *         -> A(2,0)  = access 3rd row, 1st column of matrix data. < this is the
 * way.
 *       - Remove (bug) a remnant of ancient code where we haven't implemented
 * NoInitMatZero: float_prec floatData[MATRIX_MAXIMUM_SIZE][MATRIX_MAXIMUM_SIZE]
 * = {{0}}; Into: float_prec
 * floatData[MATRIX_MAXIMUM_SIZE][MATRIX_MAXIMUM_SIZE]; We can get very nice
 * speed up (MPC benchmark @2020-04-27) from 698 us to 414 us!
 *       - Implement copy constructor               (more sweet speed up, 414 us
 * -> 382 us!).
 *       - Implement assignment operator       (more more sweet speed up, 382 us
 * -> 327 us!).
 *       - Remove Copy() function, use assignment operator instead.
 *       - Change MATRIX_USE_BOUND_CHECKING -> MATRIX_USE_BOUNDS_CHECKING (see
 * ...BOUND*S*_CH...).
 *
 *    v0.8 (2020-03-26), {PNb}:
 ...
 *      - Tambahkan operator overloading untuk operasi negatif matrix (mis. a =
 * -b).
 ...
 *      - Tambahkan pengecekan index selalu positif di
 * MATRIX_PAKAI_BOUND_CHECKING.
 *
 *    v0.4 (2020-01-10), {PNb}:
 ...
 *      - Memperbaiki bug pengecekan MATRIX_PAKAI_BOUND_CHECKING pada indexing
 * kolom.
...
public:
  ...
  Matrix(const int16_t _i16row, const int16_t _i16col,
         const InitZero _init = InitMatWithZero);
  /* Init matrix size _i16row x _i16col with entries initData */
  Matrix(const int16_t _i16row, const int16_t _i16col,
         const float_prec *initData, const InitZero _init = InitMatWithZero);
  ...
  /* ------------------------------------------- Matrix entry accessing
   * functions ------------------------------------------- */
  /* For example: A(1,2) access the 1st row and 2nd column data of matrix A <---
   * The preferred way to access the matrix */
  ...
  Matrix InsertSubMatrix(const Matrix &_subMatrix, const int16_t _posRow,
                         const int16_t _posCol);
  Matrix InsertSubMatrix(const Matrix &_subMatrix, const int16_t _posRow,
                         const int16_t _posCol, const int16_t _lenRow,
                         const int16_t _lenColumn);
  Matrix InsertSubMatrix(const Matrix &_subMatrix, const int16_t _posRow,
                         const int16_t _posCol, const int16_t _posRowSub,
                         const int16_t _posColSub, const int16_t _lenRow,
                         const int16_t _lenColumn);
  /* ---------------------------------------------------- Big operations
   * ---------------------------------------------------- */
  ...
  /* Return the vector (Mx1 matrix) correspond with the diagonal entries of
   * 'this' */
  Matrix GetDiagonalEntries(void) const;
  /* Do the Cholesky Decomposition using Cholesky-Crout algorithm, return 'L'
   * matrix */
...
private:
   ...
   *  0 <= i16row <= MATRIX_MAXIMUM_SIZE      ; i16row is the row of the matrix.
   * i16row is invalid if (i16row == -1) 0 <= i16col <= MATRIX_MAXIMUM_SIZE ;
   * i16col is the column of the matrix. i16col is invalid if (i16col == -1)
   *
   * Accessing index start from 0 until i16row/i16col, that is:
   *  (0 <= idxRow < i16row)     and     (0 <= idxCol < i16col).
   * There are 3 ways to access the data:
   *  1. A[idxRow][idxCol]          <-- Slow, not recommended, but make a cute
   * code. With bounds checking.
   *  2. A(idxRow, idxCol)          <-- The preferred way. With bounds checking.
   *  3. A._at(idxRow, idxCol)      <-- Just for internal function usage.
   * Without bounds checking.
   *
   * floatData[MATRIX_MAXIMUM_SIZE][MATRIX_MAXIMUM_SIZE] is the memory
   * representation of the matrix. We only use the first i16row-th and first
   * i16col-th memory for the matrix data. The rest is unused.
   *
   * This configuration might seems wasteful (yes it is). But with this, we can
   * make the matrix library code as cleanly as possible (like I said in the
   * github page, I've made decision to sacrifice speed & performance to get
   * best code readability I could get).
   *
   * You could change the data structure of floatData if you want to make the
   * implementation more memory efficient.
   */
  int16_t i16row;
  int16_t i16col;
   ...
   *  TODO: For Matrix member function we could do the bound checking once at
   * the beginning of the function, and use this to access the floatData instead
   * of (i,j) operator. From preliminary experiment doing this only on
   * elementary operation (experiment @2020-04-27), we can get up to 45%
   * computation boost!!! (MPC benchmark 414 us -> 226 us)!
   */
  float_prec &_at(const int16_t _row, const int16_t _col) {
    return this->floatData[_row][_col];
  }
  float_prec _at(const int16_t _row, const int16_t _col) const {
    return this->floatData[_row][_col];
  }
};
...
inline Matrix::Matrix(const int16_t _i16row, const int16_t _i16col,
                      const InitZero _init) {
  this->i16row = _i16row;
  this->i16col = _i16col;
...
inline Matrix::Matrix(const int16_t _i16row, const int16_t _i16col,
                      const float_prec *initData, const InitZero _init) {
  this->i16row = _i16row;
  this->i16col = _i16col;
...
inline Matrix::Matrix(const Matrix &old_obj) {
  /* For copy contructor, we only need to copy (_i16row x _i16col) submatrix,
   * there's no need to copy all data */
...
inline Matrix &Matrix::operator=(const Matrix &obj) {
  /* For assignment operator, we only need to copy (_i16row x _i16col)
   * submatrix, there's no need to copy all data */
...
inline float_prec Matrix::operator()(const int16_t _row,
                                     const int16_t _col) const {
#if (defined(MATRIX_USE_BOUNDS_CHECKING))
  ASSERT((_row >= 0) && (_row < this->i16row) && (_row < MATRIX_MAXIMUM_SIZE),
         "Matrix index out-of-bounds (at row evaluation)");
  ASSERT((_col >= 0) && (_col < this->i16col) && (_col < MATRIX_MAXIMUM_SIZE),
 ...
 * Modified to be lvalue modifiable (I know this is so dirty, but it makes the
 * code so FABULOUS XD)
 */
...
#warning ("Matrix bounds checking is disabled... good luck >:3");
#endif
  return Proxy(floatData[_row],
               this->i16col); /* Parsing column index for bound checking */
}
...
/* ------------------------------------------ Matrix elementary operations
 * ------------------------------------------ */
/* TODO: We could do loop unrolling here for elementary, simple, and matrix
 * insertion operations. It *might* speed up the computation time up to 20-30%
 * for processor with FMAC and cached CPU (I still mull on this because the code
 * will be awful).
 */
...
inline void Matrix::vSetToZero(void) { this->vSetHomogen(0.0); }

inline void Matrix::vSetRandom(const int32_t _maxRand, const int32_t _minRand) {
  for (int16_t _i = 0; _i < this->i16row; _i++) {
    for (int16_t _j = 0; _j < this->i16col; _j++) {
      (*this)(_i, _j) =
          float_prec((rand() % (_maxRand - _minRand + 1)) + _minRand);
    }
  }
}
...
inline void Matrix::vSetIdentity(void) { this->vSetDiag(1.0); }
 ...
 *      [A20  B20  A22]
 */
inline Matrix Matrix::InsertVector(const Matrix &_Vector,
                                   const int16_t _posCol) {
  Matrix _outp(*this);
  if ((_Vector.i16row > this->i16row) ||
      (_Vector.i16col + _posCol > this->i16col)) {
    /* Return false */
    _outp.vSetMatrixInvalid();
    return _outp;
  }
...
/* Insert the _lenRow & _lenColumn submatrix, start from _posRowSub & _posColSub
 * submatrix; into matrix at the matrix's _posRow and _posCol position.
 *
 * Example: A = Matrix 4x4, B = Matrix 2x3
 *
 ...
 */
inline bool
Matrix::bMatrixIsPositiveDefinite(const bool checkPosSemidefinite) const {
 ...
 *      L = A.CholeskyDec();
 *
 *      CATATAN! NOTE! The symmetry property is not checked at the beginning to
 * lower the computation cost. The processing is being done on the lower
 * triangular component of _A. Then it is assumed the upper triangular is
 * inherently equal to the lower end. (as a side note, Scilab & MATLAB is using
 * Lapack routines DPOTRF that process the upper triangular of _A. The result
 * should be equal mathematically if A is symmetry).
 */
 ...
 */
inline Matrix Matrix::HouseholderTransformQR(const int16_t _rowTransform,
                                             const int16_t _colTransform) {
  float_prec _tempFloat;
  float_prec _xLen;
  float_prec _x1;
 ...
 *                      A = Q*R
 *
 * PERHATIAN! CAUTION! The matrix calculated by this function are Q' and R (Q
 * transpose and R). Because QR Decomposition usually used to calculate solution
 * for least-squares equation (that need Q'), we don't do the transpose of Q
 * inside this routine to lower the computation cost (user need to transpose
 * outside if they want Q).
 *
 * Example of using QRDec to solve least-squares:
 *                      Ax = b
 *                   (QR)x = b
 *                      Rx = Q'b    --> Afterward use back-subtitution to solve
 * x
 */
inline bool Matrix::QRDec(Matrix &Qt, Matrix &R) const {
  ...
  if ((this->i16row < this->i16col) || (!Qt.bMatrixIsSquare()) ||
      (Qt.i16row != this->i16row) || (R.i16row != this->i16row) ||
      (R.i16col != this->i16col)) {
  ...
  R = (*this);
  Qt.vSetIdentity();
  for (int16_t _i = 0; (_i < (this->i16row - 1)) && (_i < this->i16col - 1);
       _i++) {
...
/* Do the back-subtitution operation for upper triangular matrix A & column
 * matrix B to solve x: Ax = B
 *
 * x = BackSubtitution(&A, &B);
 *
 * CATATAN! NOTE! To lower the computation cost, we don't check that A is a
 * upper triangular matrix (it's assumed that user already make sure of that
...
/* Do the forward-subtitution operation for lower triangular matrix A & column
 * matrix B to solve x: Ax = B
 *
 * x = ForwardSubtitution(&A, &B);
 *
 * CATATAN! NOTE! To lower the computation cost, we don't check that A is a
 * lower triangular matrix (it's assumed that user already make sure of that
 * before calling this routine).
 */
inline Matrix Matrix::ForwardSubtitution(const Matrix &A,
