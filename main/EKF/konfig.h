...
#define float_prec_ZERO_ECO                                                    \
  (1e-5) /* 'Economical' zero, for noisy calculation where 'somewhat zero' is  \
            good enough */
#elif (FPU_PRECISION == PRECISION_DOUBLE)
#define float_prec double
#define float_prec_ZERO (1e-13)
#define float_prec_ZERO_ECO                                                    \
  (1e-8) /* 'Economical' zero, for noisy calculation where 'somewhat zero' is  \
            good enough */
#else
#error ("FPU_PRECISION has not been defined!");
#endif

/* Set this define to choose system implementation (mainly used to define how
 * you print the matrix via the Matrix::vPrint() & Matrix::vPrintFull()
 * function) */
...
/* ASSERT is evaluated locally (without function call) to lower the computation
 * cost */
